...
export default async function AdminDashboard() {
  ...
  const colorMap: Record<string, string> = {
    blue: 'bg-retro-sky text-ink',
    amber: 'bg-retro-lemon text-ink',
    green: 'bg-retro-mint text-ink',
    purple: 'bg-retro-grape text-ink',
    slate: 'bg-bg2 text-ink',
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        {cards.map(card => (
          <Link key={card.title} href={card.href}
            className="bg-paper rounded-xl border border-ink p-5 shadow-sm hover:shadow-md hover:border-ink transition-all"
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-mute">{card.title}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-paper rounded-xl border border-ink p-5 shadow-sm">
          ...
          <div className="space-y-2">
            {[
              ...
              <Link key={href} href={href}
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-bg2/50 transition-colors text-sm text-ink hover:text-ink"
              >
        ...
        {pendingCount > 0 && (
          <div className="bg-retro-lemon/30 border border-ink/40 rounded-xl p-5">
            ...
            <Link href="/admin/users" className="inline-flex items-center gap-1.5 text-sm font-medium text-ink2 hover:text-ink transition-colors">
