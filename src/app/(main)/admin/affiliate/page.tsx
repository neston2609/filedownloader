...
export default function AffiliatePage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-3xl font-bold text-slate-900 mb-2">Affiliate Settings</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <label className="block font-medium text-slate-800 mb-1.5 flex items-center gap-2">
        ...
        <input
          ...
          className="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 mb-4"
        />
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mt-6">
        <p className="text-sm text-amber-800 font-medium mb-1">Browser Popup Note</p>
        <p className="text-sm text-amber-700">
          The affiliate link opens via <code className="bg-amber-100 px-1 rounded text-xs">window.open(..., &apos;_blank&apos;)</code> triggered directly by the user&apos;s click, so popup blockers should not interfere. Members see a brief note explaining the new tab before their download starts.
        </p>
      </div>
    </div>
  )
